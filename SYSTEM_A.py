...
import subprocess
subprocess.run(["pip", "install", "vegafusion[embed]>=1.5.0", "vl-convert-python>=1.6.0"], check=True)
...
# Cast to str first to avoid mixed float/string types that break vegafusion
...
# =========================================================
# base bars (all victims)
crime_base = (
    alt.Chart(visual_df)
    .mark_bar(color=soft_lilac)
    .encode(
        y=alt.Y("crime_category:N", sort="-x", title="Crime Category"),
        x=alt.X(
            "count()",
...
# highlighted subset (age brush)
crime_highlight = (
    alt.Chart(visual_df)
    .mark_bar(color=viridis_purple)
    .encode(
...
time_chart = time_area + time_line
