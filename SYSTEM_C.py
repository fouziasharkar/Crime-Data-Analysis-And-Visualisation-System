...
def make_age_heatmap_spec(df: pd.DataFrame) -> dict:
    ...
    chart = (
        ...
        .encode(
            ...
            color=alt.Color("count:Q", title="Victims",
...
function mapSpec(data) {{
  ...
  spec.params = [{{
    name: "mapBrush",
    select: {{ type: "interval", encodings: ["x","y"], translate: true, zoom: false }}
  }}];
  spec.encoding.opacity = {{
    condition: {{ param: "mapBrush", value: 0.85 }},
    value: 0.15
  }};
  spec.encoding.size = {{
    condition: {{ param: "mapBrush", value: 30 }},
    value: 12
  }};
  return spec;
}}
...
function sexSpec(data) {{
  ...
  spec.encoding.strokeWidth = {{ condition:{{ param:"sexSel", value:2 }}, value:0 }};
  spec.encoding.stroke      = {{ condition:{{ param:"sexSel", value:"#333" }}, value:"transparent" }};
  // remap fields from Altair names
